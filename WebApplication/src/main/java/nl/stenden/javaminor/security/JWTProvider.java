...
public class JWTProvider {
    ...
    public String createToken(String username) {
        ...
        return Jwts.builder().setSubject(username).setIssuedAt(now).setExpiration(expiration).signWith(ALGORITHM, secretKey).compact();
    }
    ...
    public String getRefreshToken(String tokenString) {
        ...
        if (new Date(new Date().getTime() + validityInMilliseconds / 10).after(expiration)) return createToken(user);
